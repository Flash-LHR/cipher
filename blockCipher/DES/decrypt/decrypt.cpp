...
class decrypt {
public:
    static vector<bool> decryption(vector<bool> C) {
        vector<bool> LR = IP::afterIP(C);
        vector<bool> L(32), R(32), temp(32), RL(64), P(64);
        for(int i = 0; i < 32; ++i) {
            L[i] = LR[i];
            R[i] = LR[i + 32];
        }
        ...
        for(int i = 0; i < 16; ++i) {
            ...
            R = F::afterF(R, keySchedule::getRoundKey(16 - i));
...
int main(int argc, char* argv[]) {
    if(argc != 2 || strlen(argv[1]) != 64) {
        ...
        vector<bool> C(64);
        for(int i = 0; i < 64; ++i) C[i] = argv[1][i] - '0';
        vector<bool> P = decrypt::decryption(C);
        cout << "plaintext:";
